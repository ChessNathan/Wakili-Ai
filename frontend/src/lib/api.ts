...
export const api = {
  ...
  google: {
    ...
    importFromDrive:  (google_doc_id: string, title?: string) =>
      req<{ document: Document; extracted_metadata: { client_name: string | null; document_title: string | null; case_number: string | null } }>('/api/google/import-from-drive', { method: 'POST', body: JSON.stringify({ google_doc_id, title }) }),
  },
};
